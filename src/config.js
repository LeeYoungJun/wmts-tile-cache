...
function loadConfig() {
  ...
  if (!path.isAbsolute(_config.cache.directory)) {
    _config.cache.directory = path.resolve(path.dirname(configPath), _config.cache.directory);
  }

  // Build a quick name → service lookup map
  _config._serviceMap = Object.fromEntries(_config.services.map(s => [s.name, s]));
